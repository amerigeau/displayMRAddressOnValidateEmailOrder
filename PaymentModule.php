            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        // LATOUTFRANCAIS - 08042021 - AFFICHER ADRESSE MONDIAL RELAY SUR FACTURE
                        if($carrier->name == 'Mondial Relay'){
                            $db = Db::getInstance();
                            $MrAddress = $db->connect()->query('
                                SELECT
                                    M.MR_Selected_Num AS mr_point_relais,
                                    M.MR_Selected_LgAdr1 AS pr_adr1,
                                    M.MR_Selected_LgAdr2 AS pr_adr2,
                                    M.MR_Selected_LgAdr3 AS pr_adr3,
                                    M.MR_Selected_LgAdr4 AS pr_adr4,
                                    M.MR_Selected_CP AS pr_cp,
                                    M.MR_Selected_Ville AS pr_ville
                                FROM '._DB_PREFIX_.'mr_selected M
                                WHERE M.id_cart = '.$this->order->id_cart.'
                            ');

                            $MrAddress = $MrAddress->fetchObject();

                            $data .= [
                                '{delivery_address1}' => $MrAddress->pr_adr1,
                                '{delivery_city}' => $MrAddress->pr_ville,
                                '{delivery_postal_code}' => $MrAddress->pr_cp,
                                '{delivery_country}' => $delivery->country,
                                '{delivery_other}' => $MrAddress->mr_point_relais,
                            ];

                            $address2 = '';
                            if($MrAddress->pr_adr2){
                                $address2 .= $MrAddress->pr_adr2;
                            }
                            if($MrAddress->pr_adr3){
                                if($MrAddress->pr_adr2)
                                    $address2 .= ' - ';
                                $address2 .= $MrAddress->pr_adr3;
                            }
                            if($MrAddress->pr_adr4){
                                if($MrAddress->pr_adr3)
                                    $address2 .= ' - ';
                                $address2 .= $MrAddress->pr_adr4;
                            }
                        ...
                        // END - LATOUTFRANCAIS - 08042021 - AFFICHER ADRESSE MONDIAL RELAY SUR FACTURE
